...
SELECT
    ...
    m.ID_Miejsca,
    (SELECT ID_Smieci FROM dbo.Smieci WHERE Czy_interwencje = 0) AS ID_Smieci, -- miary sa uzupelnianie pozniej
...
LEFT JOIN dbo.Opis_patrolu op ON 
    op.Nr_rejestracyjny = p.Nr_rejestracyjny
    AND op.Nr_odznaki_kierowcy = p.Nr_odznaki_kierowcy
    AND op.Nr_odznaki_partnera = p.Nr_odznaki_partnera
...
LEFT JOIN dbo.Miejsce m ON m.Dzielnica = p.Dzielnica_patrolu;
GO
